...
function Itinerary(props) {
    ...
    const url = "http://nakama-server1.herokuapp.com/itinerary/getCurrentItinerary"
    ...
    const uri = 'http://nakama-server1.herokuapp.com/itinerary/getDayPlans?Itinerary_identifier='+Itinerary_identifier 

    let {isPending, data, error} = useFetch(uri);
    
    useEffect(() => {
        if ((error || (data == undefined && error == ""))) {
            getCache('dayplans', 'http://nakama-server1.herokuapp.com').then((item) => {
                setDayPlans(item);
            });
        }
            ...
            addCache('dayplans', 'http://nakama-server1.herokuapp.com', data);
        }
    }, [data])
